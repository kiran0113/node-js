...
const B2CReceiveFees: React.FC<any> = ({ partnerDeliveryType, updateBtnShow, onNextClick }) => {
  const [currency, setCurrency] = useState(null);
  const [showcurrency, setShowCurrency] = useState(null);
  const { partnerid, type } = useParams();
  const PartnerTypeStaticValue = "B2C";
  ...
  const onAddClick = () => {
    ...
    const rtpreceivefeesapimodel = rtpreceivefeeModel.filter(
      (receive) => receive.receiveFixedFees !== null
    );
